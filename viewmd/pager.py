...
def display_directory_listing(
    dir_path: str, *, no_pager: bool, width: int, color: bool, depth: int = 1
) -> None:
    ...
    issue)."""
    if not should_page(no_pager):
        print(render_directory_listing(dir_path, width=width, color=color, depth=depth), end="")
        return

    from viewmd.interactive_pager import run_directory_listing

    run_directory_listing(dir_path, width=width, color=color, depth=depth)
