...
1. Read the issue file (`issues/VIEWMD-NNNN-*.md`). Confirm the Definition of Ready
   ...
   `sdlc-new-issue` skill's acceptance step first -- do not start a worktree for an issue that
...
`./run-tests.sh` is green, use the `sdlc-land-issue` skill.
