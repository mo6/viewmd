...
**A rich `MarkdownElement` container only renders once its own closing token is reached — a token slice that stops mid-container renders as nothing at all, not a truncated version of it.** VIEWMD-0104's mark-highlighting measured a marked region's rendered line span by rendering successive real *prefixes* of the document's tokens into throwaway buffers and diffing line counts. VIEWMD-0106 needed to extend that to a region nested inside a list item, and the naive approach — cut the token slice at the item boundary — produced empty output every time: `ListElement`/`TableElement`-style container elements absorb their children via `on_child_close` and only actually `yield` their rendered content when their own *closing* token is processed, so a prefix ending mid-list never reaches that point. The fix was reconstructing a valid, properly-closed partial container (re-attaching the container's own closing token around a truncated run of its children) rather than a bare slice — verified empirically against the real render, not assumed correct by analogy. A second, subtler trap surfaced from the same root cause: a *reconstructed* partial container isn't always equivalent to how the real document would render that same partial content, whenever the container's rendering depends on more than just "concatenate my children" — a table's box-drawn border only draws once, at the whole table's true start/end, so a table row was excluded from this recursion entirely (reconstructing a partial table draws its border prematurely, silently mismeasuring everything after); an admonition-styled blockquote has the identical hazard for the same reason. An independent review then caught a third, even subtler instance: an *ordered* list's numbering-column width is derived from `len(self.items)` — the item count actually present in whatever's being rendered, not the document's real total — so a truncated reconstruction can compute a narrower/wider numbering column than the real list, silently shifting content-wrap points and, worse than the table case, capable of tinting the *wrong* line rather than just failing to tint one. The general lesson: before reconstructing a partial version of *any* renderable for a measurement/probing purpose, ask whether that renderable's own output ever depends on information beyond just its own included children (a shared border, a count-derived column width, anything computed once and applied uniformly) — if so, a naive partial reconstruction will diverge from the real thing in exactly the cases that matter most.

**A markdown-it block token's own `.map` cannot always be trusted to cover everything nested inside it.** VIEWMD-0106's recursive region-matching needed each candidate container's true line span to decide whether a region sat entirely inside it. `bullet_list_open`/`table_open` behave as expected — their `.map` spans their full nested content — but `list_item_open`'s `.map` covers only its *first* child's lines, not a further block that follows within the same item (a sub-list under an item that also has its own paragraph text, discovered via a real 2-level-nested-list repro that silently failed to match until this was fixed). The fix scans every token across a group's whole span for its own `.map`, taking the min start / max end across all of them, rather than trusting just the group's first token. When line-range information from a parser's own token metadata seems authoritative, check whether it was verified against the *specific* nested-content shape being handled, not just the simple/flat case that happened to be tested first.

**A hardcoded relative path inside a maintainer-tooling script can silently drift from where the referenced file actually lives, and only a live run — not a diff read — catches it.** VIEWMD-0110: `tools/worktree.sh`'s `worktree add` called `"$here/install_hooks.sh"` to wire up the pre-push hook after bootstrapping a new worktree's `.venv`, but `install_hooks.sh` actually lives at `tools/install_hooks.sh` — the call failed with "No such file or directory" and exited non-zero *before* printing the command's own "ready at ..." message, so the failure looked like the whole worktree bootstrap had crashed rather than one specific known-fixable typo. First surfaced mid-VIEWMD-0109 (worked around by manually running `./tools/install_hooks.sh` from the new worktree), then fixed as its own issue. The fix's regression test doesn't re-run a live worktree bootstrap (too slow for the pytest gate) — instead it statically parses the script for the path it invokes and asserts that path resolves to a real file, pinning the exact bug class (a path string drifting from reality) without needing a live worktree creation to catch it next time.
