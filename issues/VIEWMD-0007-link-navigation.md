...
**Proof-of-concept (2026-08-16):** [`poc/pager/pager_poc.py`](../poc/pager/pager_poc.py) de-risks the architecture-shape question raised above -- whether viewmd can own an interactive scrolling loop directly (raw terminal input, line-based scroll, xterm SGR mouse-wheel reporting, an overlay popup that suspends and resumes scrolling) using only the project's existing dependencies (`rich`, stdlib -- no `curses`, no `prompt_toolkit`), rather than delegating the whole terminal to an external `less` subprocess the way `viewmd/pager.py` does today. It renders a real Markdown file with the shipped `render_markdown`/`heading_outline` (the same VIEWMD-0062 outline data v1's popup reuses as-is) and drives scroll position, mouse wheel, and a bordered ToC popup (open/select/confirm/cancel, matching the mockup below) against a real terminal via `\x1b[?1049h` alternate-screen and `\x1b[?1000h\x1b[?1006h` mouse-reporting escapes. Confirms the approach is mechanically viable with what viewmd already depends on; it does not implement any of this issue's actual requirements (no `--no-pager`/config wiring, no tests, no keybinding polish) and is not part of the shipped package. Run `python3 poc/pager/pager_poc.py` for the interactive loop, or `python3 poc/pager/pager_poc.py --self-check` for a non-interactive sanity check of the rendering/outline/overlay machinery.
