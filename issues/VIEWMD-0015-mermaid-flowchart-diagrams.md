...
- **George Moses** (maintainer), 2026-08-05: reviewed the rendered output directly (not just the diff), against `docs/mermaid-examples.md` and `docs/example.md`'s live examples. Found several visually surprising rendering cases (subgraph-border/backward-edge overlap, obstacle-routing taking more corners than necessary, the ASCII sequence-diagram cross-head glyph, nested-subgraph node membership, parallel-edge collision beyond two duplicates) and questioned each in turn; each was independently confirmed byte-for-byte against the real `mermaid-ascii` binary before being accepted as a genuine upstream limitation rather than a viewmd-introduced bug, and filed as its own deliberate-divergence follow-up (VIEWMD-0022/0023/0024/0025/0026/0027/0028) rather than blocking this issue. Approved. "close issue 15."
