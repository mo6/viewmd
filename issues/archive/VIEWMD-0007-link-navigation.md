...
commits: [08d899d, 2d57298, a3cab60]
related: [VIEWMD-0006, VIEWMD-0061, VIEWMD-0062, VIEWMD-0067, VIEWMD-0070]
supersedes: []
changelog: "[1.31.0]"
reason:
---
...
- **Claude Sonnet 5** (agent, implementer's own pass), 2026-08-17: Implemented `viewmd/interactive_pager.py` (the owned scrolling loop) and `viewmd/pager.py`'s `display_document()` (the plain-print/`$PAGER`-override/interactive dispatch), wiring both into `viewmd/__main__.py`'s single-document branch via a new `_resolve_document()`. Ported directly from `poc/pager/pager_poc.py`, which had already been hands-on tested extensively across many rounds of real bugs found and fixed during its own development (the "[K" erase bug, the popup-splice reflow bug, the background-disappears padding bug, the color/plain Mermaid-width-divergence bug, the pending-wrap-cursor `_CLEAR_EOL` bug, several truncation-marker mismatches) -- none of those regressed in the port, verified by carrying the POC's own coverage into `tests/test_interactive_pager.py` as proper pytest tests (52 tests over every pure helper function) plus 6 new `tests/test_pager.py` tests for `display_document`'s three-way dispatch. `./run-tests.sh` green (1008 tests, ruff, pip-audit, issues index). Found and fixed one real integration gap the POC itself never hit (it was only ever exercised with a file-path argument): reading keyboard input from `sys.stdin` instead of `/dev/tty` would silently break keyboard control for piped input (`cat file.md | viewmd -`), since stdin is already fully drained by the time the pager starts -- `run()` now opens `/dev/tty` directly, the same reason external pagers like `less` do, with a graceful plain-print fallback if no controlling terminal exists at all (covered by `test_run_reads_input_from_dev_tty_not_stdin`/`test_run_falls_back_to_plain_print_without_a_controlling_terminal`). One known, accepted gap: an exception during document setup (between opening `/dev/tty` and entering the main loop, e.g. a `render_markdown` failure) would leak the `/dev/tty`/resize-pipe file descriptors and leave the `SIGWINCH` handler installed rather than being caught by the existing `try`/`finally` -- not hardened against, since the process exits immediately on any uncaught exception either way (a short-lived CLI invocation, not a long-running process where the leak would accumulate), and the terminal itself is never left in a corrupted state by this specific failure window (raw mode/alternate-screen only start after that point). Not independently verified in a real terminal (no tty available to this agent) -- the raw-terminal loop itself (mouse events, resize, actual keystrokes) needs a maintainer smoke test before this is considered fully verified, matching the Acceptance section's own "manual terminal verification (not mockable)" line.
- **George Moses** (maintainer), 2026-08-17: Smoke-tested in a real terminal and signed off ("commit"), confirming the raw-terminal loop this agent could not verify itself actually works. Found one real bug in doing so: a genuinely double-width character (an emoji, e.g. `README.md`'s own "🖼 viewmd demo" image-alt-text line) throws off the ToC/help popup's column alignment, since the interactive pager counts every character as one terminal column (`len()`, no `wcwidth`) -- a known, disclosed limitation at the time (see `_ansi_slice`'s docstring), now confirmed as a real, visible bug rather than a theoretical one. Filed as [VIEWMD-0070](../VIEWMD-0070-interactive-pager-wide-characters.md) rather than blocking this issue's landing, since it's a pre-existing, disclosed limitation, not a regression this issue introduced silently.
