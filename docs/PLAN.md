...
Rich ships a `Console.pager()` context manager, but it pages unconditionally and doesn't give control over the tty-detection policy this tool wants (page only when *our* stdout is a terminal, honor `--no-pager`, honor a user's `$PAGER` override). `pager.py` implements that policy directly: `should_page()` is `sys.stdout.isatty() and not no_pager_flag`. A multi-file concatenation or a directory listing (`display()`) still spawns `$PAGER` (default `less -R -F -X --mouse`) as a subprocess, feeding it the already-rendered ANSI text, mirroring how `git`'s pager decision works. A single Markdown document (`display_document()`) still honors an explicit `$PAGER` override the same way, but otherwise (VIEWMD-0007) owns the terminal itself via `interactive_pager.run()` instead of spawning `less` -- see the next section for why.

## Interactive pager: own the terminal directly, not `less` by default (VIEWMD-0007)
