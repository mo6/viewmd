...
#
#   ./run-tests.sh                 the whole gate: pytest, ruff, and issues/ lint+index check.
#                                   Runs every step even if an earlier one fails, so one run
#                                   shows every problem, and exits non-zero if any step failed.
